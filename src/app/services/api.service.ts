...
export class ApiService {
  ...
  getcities(lat, lng): Observable<string> {
    return this.http.get<string>(`${environment.baseurl}find?lat=${lat}&lon=${lng}&cnt=20&appid=${environment.ApiKey}`);
  }
  getdetails(name) {
    return this.http.get<string>(`${environment.baseurl}forecast?q=${name}&units=metric&appid=${environment.ApiKey}`);
  }
